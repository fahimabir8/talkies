...
export class OpenAIResponseHandler {
    ...
    private handleStopGenerating = async (event: Event) => {
        ...
        if (!this.openai || !this.openAiThread || !this.run_id){
            console.error("Missing dependencies to stop generating");
            return;
        }
        try{
    ...
    private handleStreamEvent = async (event: OpenAI.Beta.Assistants.AssistantStreamEvent) => {
        ...
        if(event.event === "thread.run.created"){
            ...
            if (event.data.step_details.type ===
                "message_creation"){
                    this.channel.sendEvent({
                        type:"ai_indicator.update",
                        ai_state: "AI_STATE_CREATING",
                        cid: cid,
                        message_id: id
                    })
                }
    ...
    private handleError = async (error: Error) => {
        ...
        await this.chatClient.partialUpdateMessage(this.message.id, {
            set: {
                text: error.message ?? "Error generating the meassage",
    ...
    private performWebSearch = async (query: string): Promise<string> => {
        ...
        try {
            ...
            console.log(`Tavily search results for query "${query}":`);
            return JSON.stringify(data);
        } catch (error) {
            console.error(`An exception occured during web serach of ${query}`);
            return JSON.stringify({
                error: "An exception occured during web serach",
            })
